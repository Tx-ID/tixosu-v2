...
export async function getAppClientToken(turso, options) {
  ...
  const refreshNeeded =
    storedToken === null ||
    storedToken.expires_on.diffNow("hours").hours < 4 ||
...
async function fetchBeatmapAttributes(clientToken, id, modsBitset) {
  ...
  const apiObject = beatmapDifficultyAttributesApiSchema.parse({
