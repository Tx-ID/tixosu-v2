...
export function create() {
  if (!config.url || !config.token)
    throw new Error(
      "upstash.js: Missing REDIS_URL or REDIS_TOKEN in environment!"
    );
...
export async function cachePlayers(upstash, data) {
  await Promise.all(
    data.map((profile) => upstash.hset(`PROFILE_${profile.osu_id}`, profile))
  );
}
 ...
 * @returns {Promise<Record<number, import("./osu").BeatmapAPIObject>>}
 */
export async function getCachedBeatmaps(upstash, beatmap_ids) {
  const results = await Promise.all(
    beatmap_ids.map((id) =>
      upstash
        .hgetall(`BEATMAP_${id}`)
        .then((result) => (result !== null ? [id, result] : null))
    )
  );
...
export async function cacheBeatmaps(upstash, data) {
  await Promise.all(
    data.map((beatmap) => upstash.hset(`BEATMAP_${beatmap.id}`, beatmap))
  );
}
 ...
 * @returns {Promise<import("./osu").BeatmapDifficultyAttributesAPIObject | null>}
 */
export async function getCachedBeatmapAttributes(
  upstash,
  beatmapId,
  modBitset
) {
  return await upstash.hgetall(
    `BEATMAP_${beatmapId.toString()}_${modBitset.toString()}`
  );
}
...
export async function cacheDifficulties(upstash, data) {
  await Promise.all(
    data.map(([beatmapId, attributes]) =>
      upstash.hset(
        `BEATMAP_${beatmapId.toString()}_${attributes.modsBitset.toString()}`,
        attributes
      )
    )
  );
 ...
 * @param {import("./osu").BeatmapDifficultyAttributesAPIObject} data
 */
export async function cacheBeatmapDifficultyAttributes(
  upstash,
  beatmapId,
  modsBitSet,
  data
  ...
  await upstash.hset(
    `BEATMAP_${beatmapId.toString()}_${modsBitSet.toString()}`,
    data
  );
}
