...
export async function overrideTimelineEvent(turso, id, value) {
    await turso.execute({
        sql: "INSERT INTO timeline_event(id, name, start_time, end_time) VALUES (:id, :name, :start_time, :end_time) ON CONFLICT (id) DO UPDATE SET start_time = :start_time, end_time = :end_time WHERE id = :id",
        args: {
            id: id,
            name: value.name,
            start_time: value.start.toISO(),
            end_time: value.start.toISO()
        }
    })
}