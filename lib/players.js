...
import { participantPublicDataBaseSchema } from "./data/models";
 ...
 * @param {import("@libsql/client/.").Client} turso
 * @param {Redis} upstash
 * @returns {Promise<import("./data/models").ParticipantPublicData[]>}
 */
export async function getPublicParticipants(turso, upstash) {
  const participants = await Turso.getVisibleParticipants(turso);
  const participantOsuProfiles = await Osu.getPlayersById(
    turso,
    upstash,
    ...participants.map((it) => it.osuId)
  );
  return participants
    .map((participant) =>
      participantOsuProfiles[participant.osuId] === undefined
        ? null
        : participantPublicDataBaseSchema.parse({
            teamId: participant.teamId,
            profile: {
              ...participantOsuProfiles[participant.osuId],
              countryCode:
                participantOsuProfiles[participant.osuId].country_code,
              avatarUrl: participantOsuProfiles[participant.osuId].avatar_url,
            },
          })
    )
    .filter((it) => it !== null);
