 ...
 * @param {z.mergeTypes<import("@/lib/data/models").RoundWithBeatmaps, { id: number | undefined }>[]} rounds
 * @returns {Promise<void>}
 */
export async function overwriteRounds(turso, rounds) {
  const existingRoundIds = await Turso.getRoundIds(turso);
  const roundsToEdit = rounds
    .map((round) => round.id)
    .reduce((set, next) => {
      if (next !== undefined) {
        set.add(next);
